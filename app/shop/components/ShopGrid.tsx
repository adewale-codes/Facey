...
export default function ShopGrid() {
  ...
  const handleAddToCart = (product: Product) => {
    // 1) add to your cart state
    // addItem({ ...product, quantity: 1 })

    // 2) optionally navigate to cart, or show toast
    router.push('/cart')
  }
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {PRODUCTS.map((p) => (
          <div key={p.id} className="group relative overflow-hidden rounded-lg shadow-lg">
            ...
            {/* BUY NOW BUTTON — calls add-to-cart */}
            ...
            <div className="mt-4 flex justify-between items-center px-5">
              ...
              <span className="text-base font-medium text-green-800">{p.price}</span>
            </div>
          </div>
        ))}
      </div>
